...
\begin{tikzpicture}
  ...
  \node[draw](Eff) at (3,-2){$
    \begin{array}{c}
      ...
      G_{ef}(\beta,\alpha)
    \end{array}$};
  \node[draw](Effd) at (6,-2){$
    \begin{array}{c}
      D_{efd}(\beta,\alpha) \\
      B_{efd}(\beta,\alpha) \\
      G_{efd}(\beta,\alpha)
    \end{array}$};
  \node[draw](Pdc) at (6,2){$P_{DC}$};
  ...
  \draw[->] (Effd) -- (Pdc) node[midway, right] {Generador FV};
  \draw[->] (Pdc) -- (Pac) node[midway, below] {Inversor DC/AC};
\end{tikzpicture}
